...
parser.add_argument('output_directory', type=str, help='path to output')
parser.add_argument('adjust_threshold', type=float, help='integer to adjust the threshold')
parser.add_argument('--run_both', action='store_true', help='boolean flag')
...
if args.output_directory == 'Select an output folder':
    directory_path = os.path.dirname(args.path)
    args.output_directory = directory_path + '/'
else:
...
def find_silences(filename):
    ...
    with wave.open(filename) as wav:
        ...
        square_threshold = max_value ** 2 * 10 ** (args.adjust_threshold / 10)
...
if args.recalculate_time_in_description:
    ...
    with open('{}_result{}'.format(*os.path.splitext(args.recalculate_time_in_description)), 'w',
...
video_track.close()
command = ['ffmpeg', '-framerate', str(frame_rate), '-s', '{}x{}'.format(width, height), '-f', 'rawvideo', '-pix_fmt',
           'rgb24', '-i', '-']
        ...
        for i in range(crossfade_length):
            ...
            for channel in range(channels):
                signal_left = int.from_bytes(left_frames[(
                                             ...
                                             'little', signed=True)
                signal_right = int.from_bytes(right_frames[i * frame_width + channel * sample_width:i * frame_width + (
...
command += ['-c:v', 'copy', '-map', '0:v:0', '-map', '1:a:0', '-y',
            '{}{}_result{}'.format(args.output_directory, filename, extension)]
subprocess.run(command)

os.unlink(audio_track.name)
os.unlink(video_track.name)

new_video_file = '{}{}_result{}'.format(args.output_directory, filename, extension)
